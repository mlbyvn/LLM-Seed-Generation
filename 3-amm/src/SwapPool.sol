...
contract SwapPool is ERC20 {
    ...
    error SwapPool__OutputExceedsReserves();
        ...
        private
    {
        ...
        // @audit wrong argument order, should be
        ...
        returns (uint256 inputAmount)
    {
        if (outputAmount < outputReserves) {
            ...
            revert SwapPool__OutputExceedsReserves();
        }
    }
        ...
        returns (uint256 inputAmount)
    {
        ...
        // If a requested output amount is close to output token reserves,
        // the output amount is slightly adjusted to prevent reverts
        // if there reserves change during execution
        if (outputAmount >= (outputReserves * 995) / 1000) {
            inputAmount = getInputAmountBasedOnOutput(outputAmount, inputReserves, outputReserves);
            _swap(inputToken, inputAmount, outputToken, (outputAmount * 990) / 1000);
        } else {
            inputAmount = getInputAmountBasedOnOutput(outputAmount, inputReserves, outputReserves);
            _swap(inputToken, inputAmount, outputToken, outputAmount);
        }
     ...
     * @dev If the user wants to swap on a 1x1 rate, add 0.05% to the output token as a slippage protection
     * @param inputToken ERC20 token to pull from caller
